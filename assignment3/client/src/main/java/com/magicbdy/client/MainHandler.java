...
public class MainHandler {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        String url = "https://jyw204s383.execute-api.us-west-2.amazonaws.com/prod";
    ...
    public static void getStats(List<Stats> statsList) throws FileNotFoundException {
        ...
        Map<Long, Long> timeToTotalLatencyMapping = new HashMap<>();
        ...
        for (int i = 0; i < statsList.size(); i++) {
            ...
            for (long timestamp : timeToLatencies.keySet()) {
                ...
                if (timeToTotalLatencyMapping.containsKey(time)) {
                    timeToTotalLatencyMapping.put(time, timeToTotalLatencyMapping.get(time) + timeToLatencies.get(timestamp));
                } else {
                    timeToTotalLatencyMapping.put(time, timeToLatencies.get(timestamp));
                }
            }
        }

        // output the stats for number of requests per second interval
        PrintWriter pw = new PrintWriter(new File("stats-throughput.csv"));
        ...
        // output the stats for number of requests per second interval
        pw = new PrintWriter(new File("stats-latency.csv"));
        ...
        sb.append('\n');
        Collections.sort(secondsIntervalList);
        startSeconds = secondsIntervalList.get(0);
        for (long sec : secondsIntervalList) {
            long cur_x_index = sec - startSeconds;
            long cur_num_request = timeToRequestMapping.get(sec);
            long cur_total_latency = timeToTotalLatencyMapping.get(sec);
            sb.append(Long.toString(cur_x_index));
            sb.append(',');
            sb.append(Long.toString(cur_total_latency / cur_num_request));
            sb.append('\n');
        }
        pw.write(sb.toString());
        pw.close();
